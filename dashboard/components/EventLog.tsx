    ...
    return (
      <div className='flex flex-col items-center justify-center h-full gap-2.5 text-center'>
        <div
          ...
          style={{ background: `${colors.line}80`, border: `1px solid ${colors.line}` }}
        >
          <Activity size={13} className='text-fg-muted/40' />
        </div>
        <p className='text-[10px] uppercase tracking-widest text-fg-muted/40 font-semibold'>
          No events
        </p>
      </div>
    );
  ...
  return (
    <div className='w-full h-full overflow-y-auto flex flex-col gap-0 pr-1'>
      {entries.map((entry, idx) => {
        ...
        return (
          <div key={entry.id} className='flex items-stretch gap-0 animate-flash-in'>
            {/* ── Timeline rail ── */}
            <div className='flex flex-col items-center w-6 shrink-0'>
              ...
              <div
                ...
                style={{
                  ...
                  boxShadow:  idx === entries.length - 1 ? `0 0 6px ${dotColor}40` : "none",
                }}
              >
                {ICON[entry.type]}
              </div>
            ...
            <div className='flex-1 min-w-0 py-1.5 pl-2 pr-1'>
              <p className='text-[11px] text-fg leading-snug'>{entry.message}</p>
              <p className='font-mono text-[9px] mt-0.5' style={{ color: `${dotColor}80` }}>
                {fmt(entry.timestamp)}
              </p>
            </div>
          </div>
