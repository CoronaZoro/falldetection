  ...
  return (
    <header className='bg-surface border-b border-line px-4 h-11 flex items-center justify-between shrink-0'>
      <div className='flex items-center gap-2'>
        ...
        <span className='font-semibold text-sm text-fg tracking-wide'>
          GUARDIAN
        </span>
      ...
      <nav className='flex items-center gap-0.5'>
        ...
        {hasActiveIncident && (
          <span className='flex items-center gap-1 px-2 py-1 rounded bg-danger/10 border border-danger/20 mr-1'>
            ...
            <span className='text-[10px] font-semibold text-danger hidden sm:inline'>
              Active incident
            </span>
          </span>
        )}
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-medium transition-colors ${
                active ? "bg-line text-accent" : "text-fg-muted hover:text-fg"
              }`}
            >
              {l.icon}
              <span className='hidden sm:inline'>{l.label}</span>
      ...
      <div className='flex items-center gap-2'>
        <span className='hidden sm:block text-xs text-fg-muted'>
          {user.name}
        </span>
        <button
