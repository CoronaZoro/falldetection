  ...
  return (
    <button
      ...
      style={{
        ...
        outline: active ? "1px solid rgba(255,255,255,0.3)" : "1px solid rgba(255,255,255,0.1)",
      }}
    >
      {icon}
      {label}
...
export default function LiveFeed({ online = true }: Props) {
  ...
  const [viz, setViz] = useState<VizFlags>({ skeleton: true, bbox: true, status_bar: true });

  // Sync initial state from server
  useEffect(() => {
    fetch(`${API_URL}/visualization`)
      .then((r) => r.ok ? r.json() : null)
      .then((d) => { if (d) setViz(d); })
      .catch(() => {});
  }, []);
